...
def _normalize_doc(doc):
    ...
    # FIXED: Map Released and Last Updated fields
...
def get_all_data():
    ...
    try:
        ...
        # FIXED: Added 'Released' and 'Last Updated' to projection
        projection = {
            '_id': 0, 
            'App Name': 1, 
            'Category': 1, 
...
def get_unique_options():
    ...
    return {'categories': categories, 'types': ['Free', 'Paid'], 'content_ratings': content_ratings}
...
def dashboard_stats():
    ...
    try:
        ...
        if not data: return jsonify({'error': 'No data available'}), 404
        
        categories = {}
        free_count = total_installs = total_reviews = 0
        for app in data:
            categories[app.get('category_clean', 'Unknown')] = categories.get(app.get('category_clean', 'Unknown'), 0) + 1
            if app.get('type', 'Free') == 'Free': free_count += 1
...
def content_rating_distribution():
    ...
    try:
        ...
        rating_installs = {}
        for app in data:
            cr = str(app.get('content_rating', 'Everyone')) or 'Everyone'
...
def category_analysis():
    ...
    try:
        ...
        for app in data:
            ...
            if app.get('type', 'Free') == 'Free': stats['free_count'] += 1
...
def top_apps():
    ...
    try:
        ...
        result = []
        for app in sorted_data[:limit]:
            result.append({
                'name': app.get('app_name', app.get('App Name', 'Unknown')),
                'category': app.get('category_clean', app.get('Category', 'Unknown')),
                'rating': app.get('rating', 0), 'installs': app.get('installs', 0), 'reviews': app.get('reviews', 0),
                'type': app.get('type', 'Free'), 'price': app.get('price_usd', app.get('Price', 0)),
...
def correlation_analysis():
    ...
    try:
        ...
        for app in data:
            if app.get('rating', 0) > 0 and app.get('reviews', 0) > 0:
                ratings.append(app.get('rating', 0))
                reviews.append(app.get('reviews', 0))
                installs.append(app.get('installs', 0))
                prices.append(app.get('price_usd', app.get('Price', 0)))
        ...
        result = {
            'correlations': {'rating_reviews': round(calc_corr(ratings, reviews), 3), 'rating_installs': round(calc_corr(ratings, installs), 3), 'reviews_installs': round(calc_corr(reviews, installs), 3)},
...
def release_year_distribution():
    """Get release year distribution. Falls back to Last Updated if Released is missing."""
    filters = {'category': request.args.get('category'), 'type': request.args.get('type'), 'content_rating': request.args.get('content_rating')}
    ...
    try:
        ...
        year_counts = {}
        for app in data:
            # Try Released first, then Last Updated as fallback
            released = app.get('released') or app.get('Released', '')
            last_updated = app.get('last_updated') or app.get('Last Updated', '')
...
def insights():
    ...
    try:
        ...
        insights_list = []
        categories = {}
        for app in data: categories[app.get('category_clean', 'Unknown')] = categories.get(app.get('category_clean', 'Unknown'), 0) + 1
